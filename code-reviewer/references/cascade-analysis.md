...
- 被修改的符号、文件和公共契约。
- 关联 issue 中的问题陈述、验收标准、显式非目标和优先级信号。
- 被修改符号的直接上游调用方和消费者。
- 受影响的执行流程和过程步骤。
- 覆盖被修改符号与受影响流程的测试。
...
- `medium`: 图谱证据覆盖关键符号，关联 issue 上下文可用，但部分边缘流程或测试依赖推断。
- `degraded`: 图谱证据不可用、过期、未映射、与 diff 矛盾，或 PR 声称关联 issue 但关联上下文缺失、截断到无法判断验收标准，或获取失败。

当置信度为 degraded 且变更触碰共享、关键或公共契约代码时，使用 `NEEDS_HUMAN`，不要使用 `APPROVE`。如果产品意图必须依赖关联 issue 才能判断，而关联 issue 上下文缺失或获取失败，也使用 `NEEDS_HUMAN`。
